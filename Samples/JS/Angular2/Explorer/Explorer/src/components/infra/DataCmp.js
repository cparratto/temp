...
var DataCmp = (function (_super) {
    ...
    function DataCmp(dataSvc) {
        ...
        _this.cv = new wjcCore.CollectionView(dataSvc.getData(500));
        _this.cv.pageSize = 10;
        _this.groupedList = _this.cv.items;
        _this.cv.filter = _this._filterFun.bind(_this);
        _this.cv.newItemCreator = function () {
            var newItem = dataSvc.getData(1)[0];
            newItem.id = -1;
            return newItem;
        };
        _this.cv.collectionChanged.addHandler(function () {
            _this.groupedList = _this.cv.items;
        ...
        _this.filter = { id: '', country: '', color: '', minAmount: null, month: null };
        return _this;
    }
