...
var GroupFooters = GroupFooters_1 = (function () {
    ...
    GroupFooters.prototype._addGroupFooters = function () {
        ...
        for (var r = 0; r < flex.rows.length; r++) {
            var row = flex.rows[r];
            if (row instanceof wjcGrid.GroupRow && row.cssClass != GroupFooters_1.ROW_FOOTER_CLASS) {
                // create footer row to match this group row
                var newRow = new wjcGrid.GroupRow();
                newRow.level = row.level;
                newRow.cssClass = GroupFooters_1.ROW_FOOTER_CLASS;
    ...
    GroupFooters.prototype._findFooterIndex = function (r) {
        ...
        for (var i = r + 1; i < flex.rows.length; i++) {
            ...
            if (row instanceof wjcGrid.GroupRow) {
                ...
                if (row.cssClass != GroupFooters_1.ROW_FOOTER_CLASS && row.level <= level) {
                    return i;
                }
...
GroupFooters = GroupFooters_1 = __decorate([
    core_1.Directive({
        selector: '[groupFooters]',
        inputs: ['groupFooters']
    }),
    __param(0, core_1.Self()), __param(0, core_2.Inject(WjFlexGrid.WjFlexGrid)),
