...
import * as wjcGridSheetChart from './wijmo/wijmo.grid.sheet.chart';
...
import { DataSvc } from './services/DataSvc';
    ...
    export class ExcellikeSheetCmp implements OnInit {
        ...
        private _updateWithImport = false;
        private _applyFillColor = false;
        private _appliedClass = '';
        private _cellStyleApplying = false;
        private _pendingAction: wjcGridSheet._UndoAction;
        private _chartEngine: wjcGridSheetChart.ChartEngine;
        ...
        // references ColorPicker named 'colorPicker' in the view
        @ViewChild('colorPicker') colorPicker: wjcInput.ColorPicker;

        // references ListBox named 'lbChartTypes' in the view
        @ViewChild('lbChartTypes') lbChartTypes: wjcInput.ListBox;
        ...
        constructor(@Inject(DataSvc) dataSvc: DataSvc) {
            ...
            document.body.addEventListener('click', () => {
                if (self.lbChartTypes) {
                    self.lbChartTypes.hostElement.style.display = 'none';
                }
            }, true);
            ...
            document.body.addEventListener('keydown', (e) => {
                if (self.lbChartTypes) {
                    ...
                    if (e.keyCode === 13 && self.lbChartTypes.hostElement.style.display !== 'none') {
                        self._addChart();
                    }
                }
            });
        ...
        flexInitialized(flexSheet: wjcGridSheet.FlexSheet) {
            var self = this;

            if (flexSheet) {
                self._chartEngine = new wjcGridSheetChart.ChartEngine(flexSheet);
                ...
                flexSheet.cellEditEnded.addHandler((sender: any, args: wjcGrid.CellRangeEventArgs) => {
        ...
        colorPickerInit(colorPicker: wjcInput.ColorPicker) {
            ...
            if (colorPicker) {
                ...
                colorPicker.hostElement.addEventListener('keydown', (e) => {
                    if (e.keyCode === 27) {
                        colorPicker.hostElement.style.display = 'none';
                    }
                });
        ...
        lbChartTypesInit(lbChartTypes) {
            var self = this;
            if (lbChartTypes) {
                lbChartTypes.hostElement.addEventListener('click', () => {
                    self._addChart();
                });
            }
        }
        ...
        applyCellStyle (className, cancelCellStyle) {
            if (cancelCellStyle) {
        ...
        showChartTypes(e) {
            var lbChartTypes = this.lbChartTypes,
                target = e.target,
                offset;
            ...
            offset = this._cumulativeOffset(target);
        ...
        private _addChart() {
            ...
            if (lbChartTypes.selectedValue != null) {
                chartType = wjcCore.asEnum(lbChartTypes.selectedValue, wjcChart.ChartType);
            }
            this._chartEngine.addChart(chartType);
