...
var DataSvc = (function () {
    function DataSvc() {
        ...
        this._musicians = 'Paul,Mark,Pete,Ringo,Luke,Jacob,John,Nate,Zym,George,Toom,Crash,Boom,Dewd'.split(',');
        this._someCountries = ['US', 'Germany', 'UK', 'Japan', 'Italy', 'Greece'];
        this._allCountries = [
    ...
    DataSvc.prototype.getSomeCountries = function () {
        return this._someCountries;
    };
    DataSvc.prototype.getAllCountries = function () {
        return this._allCountries;
    };
    DataSvc.prototype.getProducts = function () {
        return this._products;
    };
    DataSvc.prototype.getColors = function () {
        return this._colors;
    };
    DataSvc.prototype.getMusicians = function () {
        return this._musicians;
    };
    DataSvc.prototype.getPalettes = function () {
    ...
    DataSvc.prototype.getData = function (count, unique) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            var date = new Date(dt.getFullYear(), i % 12, 25, i % 24, i % 60, i % 60), countryId = unique == true ? i : Math.floor(Math.random() * this._someCountries.length), productId = Math.floor(Math.random() * this._products.length), colorId = Math.floor(Math.random() * this._colors.length);
            // create the item
            var item = {
                id: i,
                start: date,
    ...
    DataSvc.prototype.initOData = function (categories, products, supplierMap) {
        ...
        $.ajax({
            ...
            url: urlSuppliers,
            success: function (data) {
                supplierMap = new wjcGrid.DataMap(data.value, 'SupplierID', 'CompanyName');
                //safeApply();
            }
        });
        ...
        categories.currentChanged.addHandler(function (s, e) {
            var id = categories.currentItem.CategoryID, urlProducts = urlBase + 'Products?$format=json&$select=ProductID,SupplierID,ProductName,QuantityPerUnit,UnitPrice,UnitsInStock,Discontinued&$filter=CategoryID eq ' + id;
            $.ajax({
                dataType: 'json',
                url: urlProducts,
                success: function (data) {
    ...
    DataSvc.prototype.getChartData = function () {
        return [
    ...
    DataSvc.prototype.getPieChartData = function () {
        // populate itemsSource
    ...
    DataSvc.prototype.getTreeMapData = function () {
        var rand = this.rand;
        var data = [{
                type: 'Music',
                items: [{
