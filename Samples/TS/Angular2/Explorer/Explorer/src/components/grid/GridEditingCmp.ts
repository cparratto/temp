...
export class GridEditingCmp extends GridBaseCmp {
    ...
    ngAfterViewInit() {
        ...
        $('#dlgDetail').on('shown.bs.modal', function (e) {
            (<wjcCore.CollectionView>self.flex.collectionView).editItem(self.flex.collectionView.currentItem);
        });
        if (this.flexInline) {
    ...
    deleteRow(rowIdx: number) {
        this.editIndex = -1;
        var ecv = <wjcCore.CollectionView>this.flexInline.collectionView;
        ecv.removeAt(rowIdx);
    }
