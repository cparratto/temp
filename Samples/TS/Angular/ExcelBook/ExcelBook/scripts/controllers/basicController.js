...
angular.module('app').controller('basicController', function ($scope, dataService) {
	...
	$scope.$watch('ctx.lbChartTypes', function () {
	    var lbChartTypes = $scope.ctx.lbChartTypes;
	    if (lbChartTypes) {
	        lbChartTypes.hostElement.addEventListener('click', addChart);
	    }
	});
	...
	$scope.applyCellStyle = function (className, cancelCellStyle) {
		if (cancelCellStyle) {
			if (cellStyleApplying) {
				...
				$scope.ctx.flexSheet.applyCellsStyle(null);
			}
		} else {
			if (className) {
				appliedClass = className + '-style';
				$scope.ctx.flexSheet.applyCellsStyle({ className: appliedClass }, undefined, true);
	...
	function applyStyleForFormulasSheet(flexSheet) {
		...
		flexSheet.applyCellsStyle({
		    format: 'n2'
		}, [new wijmo.grid.CellRange(229, 1, 232, 8),
			new wijmo.grid.CellRange(392, 5, 401, 5)]);
