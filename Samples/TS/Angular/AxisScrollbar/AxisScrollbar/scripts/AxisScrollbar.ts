module wijmo.chart.interaction {
    ...
    export class AxisScrollbar {
        ...
        private _axis: wijmo.chart.Axis = null;
        ...
        private _chartRefreshDelay = null;
         ...
         */
        constructor(axis: wijmo.chart.Axis, options?) {
            if (!chart) {
                assert(false, 'The Axis cannot be null.');
            }

            this._axis = axis;
            this._chart = axis._chart;
            this._isXAxis = this._axis.axisType === AxisType.X;
      ...
      private  _createScrollbar() {
            ...
            this._slbarContainer = createElement('<div class="wj-axis-scrollbar-container"></div>');
            this._rangeSlider = new _RangeSlider(this._slbarContainer,
        ...
        private _refresh() {
            var chartHostEle = this._chart.hostElement, 
                ...
                rOffset = wijmo.getElementRect(this._slbarContainer);
            ...
            if (this._min === null) {
                this._min = isDate(this._axis.actualMin) ? this._axis.actualMin.valueOf() : this._axis.actualMin;
            }
            if (this._max === null) {
                this._max = isDate(this._axis.actualMax) ? this._axis.actualMax.valueOf() : this._axis.actualMax;
            }
            ...
            if (this._isXAxis) {
                isBottom = this._axis.position === Position.Bottom;
                ...
                isLeft = this._axis.position === Position.Left;
